...
# Bench cpu-2-gpu
python profile_tensor_transform.py $MODEL $MOBILE 2
adb push ../models/$MODEL/$DEVICE_MAP_FILE1 $ANDROID_DIR
adb shell "LD_LIBRARY_PATH=$ANDROID_DIR:$LD_LIBRARY_PATH $ANDROID_DIR/benchmark.out $ANDROID_DIR/mnn_models/mnn_$MODEL 7 3 1 2 1 $ANDROID_DIR/$DEVICE_MAP_FILE1 > $ANDROID_DIR/tmp.txt"
...
adb shell "LD_LIBRARY_PATH=$ANDROID_DIR:$LD_LIBRARY_PATH $ANDROID_DIR/benchmark.out $ANDROID_DIR/mnn_models/mnn_$MODEL 7 3 1 2 1 $ANDROID_DIR/$DEVICE_MAP_FILE2 > $ANDROID_DIR/tmp.txt"
adb shell "cat $ANDROID_DIR/tmp.txt | grep TensorConvert | grep copyToDevice >> $ANDROID_DIR/$MODEL-$MOBILE-c2g-multi-data-trans.txt"
adb shell "cat $ANDROID_DIR/tmp.txt | grep TensorConvert | grep copyFromDevice >> $ANDROID_DIR/$MODEL-$MOBILE-g2c-multi-data-trans.txt"
...
python data_trans_generate.py $MODEL $MOBILE $THREAD
