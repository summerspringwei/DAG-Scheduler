...
def avg_data_trans(model, mobile):
    ...
    profile_dict_c2g = read_multi_runs_latency(multi_data_trans_file_path)
    file_name = model+'-'+mobile+'-g2c-multi-data-trans.txt'
    multi_data_trans_file_path = os.path.join("../models", model, mobile, file_name)
    profile_dict_g2c = read_multi_runs_latency(multi_data_trans_file_path)
    ...
    for name, values in profile_dict_c2g.items():
        trans_latency_c2g = np.average(filter_list(values[2]))
        if name in profile_dict_g2c.keys():
            trans_latency_g2c = np.average(filter_list(profile_dict_g2c[name][2]))
        else:
