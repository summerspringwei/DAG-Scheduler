...
class Subgraph(Operator):
    ...
    def _summaryLatency(self, global_name_op_dict):
        self.op_def.operatorLatency.CPU_latency = 0
        self.op_def.operatorLatency.GPU_latency = 0
        for op_name in self.op_name_list:
            op = self.name_op_dict[op_name]
        ...
        self.op_def.operatorLatency.Transpose_latency_NHWC_to_NCHW = 0.
        self.op_def.operatorLatency.Transpose_latency_NCHW_to_NHWC = 0.
        ...
        for (addr, latency) in self.op_def.operatorLatency.input_data_trans_latency.items():
            self.op_def.operatorLatency.Transpose_latency_NHWC_to_NCHW += float(latency[0])
            self.op_def.operatorLatency.Transpose_latency_NCHW_to_NHWC += float(latency[1])
        # Debug info
    ...
    def __str__(self):
        ...
        return ("name: %s\nlatency: %s\nnodes:%s\nparents:%s\nchildren:%s\ninput_tensors_map%s\ninput_tensors%s\noutput_tensors%s\n" \
            %(self.name, str_latency, self.op_name_list, self.parents, self.children,\
                self.op_def.operatorLatency.input_data_trans_latency, self.input_tensors, self.output_tensors))
